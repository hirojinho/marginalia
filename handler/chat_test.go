...
func TestChatRejectsMissingMessage(t *testing.T) {
	h := newTestHandler(t)
	sess, err := h.App.CreateSession("biology", "test", "scratch")
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	body, _ := json.Marshal(chatRequest{SessionID: sess.ID, Message: ""})
	req := httptest.NewRequest(http.MethodPost, "/chat", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	h.handleChat(w, req)
...
func TestChatRejectsMethodNotPost(t *testing.T) {
	h := newTestHandler(t)
...
func TestChatReturnsPiUnconfiguredWhenNoPiPath(t *testing.T) {
	h := newTestHandler(t) // Config.PiPath is empty by default
	sess, err := h.App.CreateSession("biology", "test", "scratch")
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	body, _ := json.Marshal(chatRequest{SessionID: sess.ID, Message: "hi"})
	req := httptest.NewRequest(http.MethodPost, "/chat", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	h.handleChat(w, req)
...
func TestChatCreatesSandboxBeforeCheckingPi(t *testing.T) {
...
func TestDeleteSessionRemovesSandbox(t *testing.T) {
	...
	sess, err := h.App.CreateSession("biology", "deletion test", "scratch")
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
...
func TestBuildPiPromptPassthroughWhenNoClawCLI(t *testing.T) {
	got := buildPiPrompt("cs101", "", "hello", nil)
	if got != "hello" {
		t.Errorf("got %q, want passthrough %q", got, "hello")
	}
}
...
func TestBuildPiPromptPassthroughWhenClawCLIFails(t *testing.T) {
	got := buildPiPrompt("cs101", "/no/such/binary/path", "hello", nil)
	if got != "hello" {
