...
- **Edit session topic.** Sessions have a topic set at creation time but no way to rename it afterwards. Add a rename path: click on the session name (or a pencil icon) in the sidebar to edit it inline; `PATCH /api/sessions/:id` persists the change. Small UI + one new endpoint.
- **Persist reasoning across reloads.** Thinking tokens (SSE `reasoning` events) are rendered during a live turn but not saved — `/chat-v2` only persists the answer text. On page reload, thinking blocks disappear. Fix: store the reasoning content alongside the assistant message (separate DB column or a `messages.reasoning` field); populate the thinking block when loading message history in `static/sessions.js`. Pairs well with the inline-reasoning-stream item above.
