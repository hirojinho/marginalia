...
- **Knowledge Component arc (designed 2026-05-27 — see [ADR 0007](docs/adr/0007-knowledge-component-as-atomic-note.md) + [CONTEXT.md](CONTEXT.md)).** Reframes the retrieval-side work around a first-class, **learner-authored atomic note** (Zettelkasten/KLI granularity) instead of the task-UUID stand-in. A study task covers several ideas, so the atom must live below the task; and the learner — not the agent — writes the note's body (the app absorbs *management* friction, the learner keeps the *cognitive* labor). Decomposed into a dependency-ordered chain of overnight tickets:
  1. **Rename `kc_id` → `knowledge_component_id` + `/debug/schema`.** *Queued* (`specs/queue/2026-05-27-rename-knowledge-component-id.md`). Vocabulary alignment + a reusable schema-introspection endpoint for verifiers.
  2. **`knowledge_components` table + DB methods + `claw-cli knowledge create|show|list`.** The atomic-note entity (`id`, `title`, `body`, `source_task_id`, `source_session_id`) + CLI inspect/create path. *Not yet queued.*
  3. **Agent `knowledge_create` tool + capture-flow pedagogy rule.** Agent proposes the title and elicits the body; the learner writes the body, stored verbatim. Weaker gate (model-driven elicitation) — go-test the handler + CLI round-trip. *Not yet queued.*
  4. **Retrieval-practice loop (was R1).** `retrieval_queue` keyed on `knowledge_component_id`, upsert on `log_confidence`, confidence-banded `due_at` (1d/3d/7d), `claw-cli retrieve due`, Rule-6 hook to open sessions with what's due. Highest single ROI (Roediger & Karpicke 2006; Karpicke & Blunt 2011, *Science*; Dunlosky et al. 2013). *Not yet queued.*
  - **Deferred specs (additive, no rework — see ADR 0007):** HTML capture box (frontend, `requires_visual_approval` — the nicer authoring surface, posts to ticket 2's create path; relates to the *Agent-emitted HTML snippets* item under Later); link graph + multi-source provenance (the synthesis-note feature; promotes provenance to its own table); per-concept visual dashboard (the *R-Later — Pedagogy dashboard* item under Later).
- **R2 — Expanding-interval spaced review (SM-2).** Extends the retrieval loop (arc ticket 4): after each retrieval, schedule next review on an SM-2-style schedule (confidence high → interval ×2.5; low → reset to 1 d). Cepeda et al. (2008, *Psychological Science*). Small marginal cost once ticket 4 lands; swaps its banded interval for real spacing.
