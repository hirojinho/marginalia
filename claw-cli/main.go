// Command claw-cli is the agent's command-line surface into marginalia state.
...
func runPlan(args []string, stdout, stderr io.Writer, dbPath string) int {
	...
	case "active":
		return planActive(args[1:], stdout, stderr, dbPath)
	case "toggle":
...
// planActive prints the first not-done task as "id<TAB>title" so the agent
// can pass it to `knowledge create --source-task-id`. It walks the plan in
// order, checking both phase-level tasks and cluster tasks. If all tasks are
// done it prints nothing and returns 0.
func planActive(args []string, stdout, stderr io.Writer, dbPath string) int {
	fs := flag.NewFlagSet("plan active", flag.ContinueOnError)
	fs.SetOutput(stderr)
	course := fs.String("course", "", "course id (required)")
	dbOverride := fs.String("db", "", "path to study.db")
	if err := fs.Parse(args); err != nil {
	...
	if *course == "" {
		_, _ = fmt.Fprintln(stderr, "plan active: --course is required")
		return 2
	}
	resolvedDB, err := resolveDBPath(*dbOverride, dbPath)
	if err != nil {
	...
	// All tasks done: print nothing, success.
	return 0
}
...
func knowledgeCreate(args []string, stdout, stderr io.Writer, dbPath string) int {
	...
	if strings.HasPrefix(*sourceTaskID, "--") {
		_, _ = fmt.Fprintf(stderr, "knowledge create: --source-task-id got %q, which looks like a flag — you likely omitted its value. Get the task id from `claw-cli plan active --course <id>`.\n", *sourceTaskID)
		return 2
	}
	resolvedDB, err := resolveDBPath(*dbOverride, dbPath)
	if err != nil {
