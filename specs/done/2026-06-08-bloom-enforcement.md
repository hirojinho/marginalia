...
if grep -q 'BloomLevel' agent/types.go; then
  echo "UNEXPECTED: BloomLevel already exists on main"
  exit 0
fi
if grep -q 'phaseCompletionBloomCheck\|bloom.*enforcement\|Bloom.*enforcement' agent/tools_plan.go; then
  echo "UNEXPECTED: Bloom enforcement already exists on main"
  exit 0
fi
if grep -q 'bloom_level.*remember.*understand.*apply.*analyze.*evaluate.*create\|analyze.*evaluate.*create.*MUST' agent/sandbox.go; then
  echo "UNEXPECTED: Bloom guidance already in sandbox"
  exit 0
fi
# All features absent — expected pre-state. Signal "fail" (non-zero).
exit 1
```
