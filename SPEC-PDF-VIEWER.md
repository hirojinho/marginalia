...
Study webapp for an ITA master's student. Stack: Go backend (`net/http`), single-page vanilla JS frontend (no framework), inline CSS, single binary deploy. No htmx — the existing app uses vanilla JS + fetch + SSE for all interactivity. No auth on local deploy.
...
CREATE TABLE IF NOT EXISTS pdfs (
    ...
    course_id   TEXT,                    -- links to knownCourses (ce297, ddia, etc.), nullable
...
The `course_id` column links PDFs to courses from the study plan (same IDs: `ce297`, `ddia`, `dsa-interview`, `software-arch`, `thesis`). When uploading, the user picks a course from a dropdown (or leaves it uncategorized).
...
GET  /pdf/list            → JSON list of all PDFs (optional ?course=ce297 filter)
...
- Course name (e.g. "Safety Models and Techniques (CE-297)")
...
Known course IDs (matching the plan system): `ce297`, `ddia`, `dsa-interview`, `software-arch`, `thesis`.
