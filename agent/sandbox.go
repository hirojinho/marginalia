...
func (sm *SandboxManager) studyTuningSections(course string) []byte {
	...
	pedagogySection := "\n## Pedagogical Rules (MANDATORY — apply on every turn)\n\n" +
		...
		rule9 +
		rule10 +
		"11. **On a partial answer, cue — don't complete.** When the learner recalls or answers *part* of something, do NOT immediately supply the missing pieces. First give a minimal cue toward the gap (a hint, a category, \"there's one more — think about X\") and let them attempt retrieval; reveal the full answer only after they try again or explicitly pass. The effort on the gap is the learning (Bjork & Bjork 1992, desirable difficulties; Slamecka & Graf 1978, generation effect). **At the session opener, cap this at ONE cue** — the opener is a warm-up, not a drill: if he's still short after one cue, fill the gap in a sentence and move into the work (the multi-round grind is banned at the opener; ADR 0020).\n" +
		rule12 +
		rule13 +
