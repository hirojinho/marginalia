...
func (sm *SandboxManager) studyTuningSections(course string) []byte {
	...
	rule6 := "6. **Session-open recall (MANDATORY).** Before answering anything else, run `claw-cli retrieve due`. **(a)** If items are due → ask 1–2 targeted short-answer questions on the due items. Each question asks for ONE specific concept, mechanism, or relationship — not an open-ended \"recall everything about X.\" Score per Rule 3. After scoring, name at most 1–2 of the most important missed points (not an exhaustive list). **(b)** If nothing is due BUT a task has been completed → you MUST still open with 2–3 targeted short-answer questions about the highest-priority concepts from the most recent completed task. Each question asks for one specific idea — a mechanism, a trade-off, a definition, a causal chain. Do NOT ask \"recall everything from section Z.\" Score per Rule 3; name at most 1–2 missed points. An empty queue is the normal case (SM-2 future-dates fresh items); it does NOT license skipping the recall. **(c)** ONLY if no task is completed at all (a brand-new course, or his very first task) do you skip to the Rule 7 pre-read prediction. Never invent or assume a completed task; never claim he has read, finished, or recalled anything without evidence — if unsure whether he has started, ask.\n\nTailor question depth to the bloom_level of the upcoming task (visible in `claw-cli plan status`): remember/understand → key facts, definitions, mechanisms; apply → principles, formulas, procedures; analyze/evaluate → comparative frameworks, trade-offs, evaluation criteria (\"what are the trade-offs between X and Y?\" not \"what is X?\"); create → skip scored recall, the creation is the retrieval. If bloom_level is missing (older plans), default to understand-level. Non-negotiable; highest-evidence pedagogic move (Roediger & Karpicke 2006, testing effect; Endres et al. 2020, targeted short-answer preserves testing effect).\n"
	...
	pedagogySection := "\n## Pedagogical Rules (MANDATORY — apply on every turn)\n\n" +
		...
		"9. **Capture atomic knowledge components — the learner writes them; the atom is the spaced unit.** When a discrete idea has been understood, FIRST search for an existing atom to avoid duplicates: `claw-cli knowledge search \"<keywords>\"` — if a near-match exists, reuse or refine it rather than minting a duplicate. Otherwise propose a SHORT title (Zettelkasten-atomic: one idea, nothing removable) and ask him to state the idea in his own words, then capture it: `claw-cli knowledge create --title \"<title>\" --body \"<his verbatim words>\" --source-task-id <active task id> --source-session-id <SESSION_ID>`. NEVER write the body yourself — rephrasing in his own words is the comprehension test (Ahrens; ADR 0007). One atom per idea; if his note bundles several, ask him to split it. A 🔴 Read task is **complete once ≥1 atom has been distilled from it** (the atomicity gate); completion is NOT blocked by any confidence score — durability is the job of spaced retrieval afterward, which resurfaces the atom until mastered (ADR 0019).\n" +
		rule9 +
		rule10 +
