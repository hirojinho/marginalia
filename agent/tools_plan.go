...
// collectTaskIDs returns a set of all task ids across phases and clusters.
func collectTaskIDs(p *JSONPlan) map[string]bool {
...
// collectTaskIDsSlice returns a slice of all task ids (order not guaranteed).
func collectTaskIDsSlice(p *JSONPlan) []string {
	if p == nil {
		return nil
	}
	var ids []string
