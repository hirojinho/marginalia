...
func TestLoadCourseInterests_MissingFile(t *testing.T) {
	a := newMemoryApp(t)
	if got := a.loadCourseInterests("biology"); got != "" {
...
func TestLoadCourseInterests_PresentFile(t *testing.T) {
	a := newMemoryApp(t)
	dir := a.VaultPath("data", "courses", "biology")
	...
	got := a.loadCourseInterests("biology")
...
func TestToolStudySkill_AllSkillBranches(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.skill, func(t *testing.T) {
			args := json.RawMessage(`{"skill":"` + tc.skill + `","params":{"topic":"STPA","course_id":"biology","count":"3"}}`)
...
func TestLoadCorpusContext_FallbackToCourseDir(t *testing.T) {
	...
	dir := a.VaultPath("data", "corpus", "courses", "biology")
	_ = os.MkdirAll(dir, 0755)
	_ = os.WriteFile(filepath.Join(dir, "intro.md"), []byte("STAMP"), 0644)
	got := a.loadCorpusContext("missing-topic-zz", "biology")
