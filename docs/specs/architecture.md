...
│   ├── tools.go           — Tool definitions + execution (7 tools)
│   ├── llm.go             — CallLLM, ProcessWithTools, ParseStream
...
- [x] Expand corpus with course concept cards (ce297: stamp, stpa, cast, process-models, coordination-failures; ddia: replication, transaction-isolation, storage-engines, consensus, partitioning, stream-processing)
