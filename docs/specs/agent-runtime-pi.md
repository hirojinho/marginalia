...
- **`marginalia`** owns auth, sessions, plans, RAG corpus, PDF state, persistence, observability, the chat UI. It spawns Pi per chat turn and translates Pi's event stream into SSE.
...
- **`claw-cli`** is a small Go binary exposing `marginalia`'s domain operations as CLI subcommands. Pi invokes it via its `bash` tool. It calls back into `marginalia` (over a localhost HTTP loopback or a shared SQLite file, depending on collocation choice in implementation).
...
Per-session ephemeral working directory at `/path/to/marginalia/data/agent-sessions/<session-id>/`:
...
3. **Active feedback memories** — style, density, voice rules relevant to this course (e.g. for Biology: diagram-first rule, no-abbreviations rule; for Algorithms: descriptive variable names, data-structure-semantics-first).
...
The memory store itself is owned by `marginalia`. New table in `study.db`:
...
Initial seed: import the relevant memories from your memory directory (the user-profile and feedback files, scoped course memories, course interests). Future writes happen via `claw-cli memory save`.
...
**Out of scope for v1:** career skills (`interview-prep`, `resume-tailoring`, `candidate-profile`, `job-application-prep`, `job-hunt`) — port later. `commit-and-push` is permanently out of scope (the agent does not ship code from `marginalia`).

## `claw-cli` surface

A Go binary at `/path/to/marginalia/bin/claw-cli`, on the agent's `$PATH`. Wraps existing `agent/tools_*.go` functions where they exist; introduces new helpers for memory and notes. All subcommands write JSON to stdout for parsing; errors go to stderr with non-zero exit.
...
The CLI is the only path agent code takes to `marginalia` state.

## Model selection

Configured via `/path/to/marginalia/.env`. Defaults:
