...
Reject Docker and Compose. Build the binary on the laptop with `GOOS=linux GOARCH=amd64 go build`, scp it to the VPS, atomic-swap with the previous version, restart the systemd unit. Operational hardening lives in the boring base layer: journald for logs, restic for nightly snapshots of `~/stack/study-app`, env vars in mode-0600 `.env`.

## Consequences

- Deploy is two commands (build + scp) and a restart. No image registry, no compose file, no daemon to keep running.
- Rollback is `mv study-app.bak study-app && systemctl --user restart` — a literal file swap.
