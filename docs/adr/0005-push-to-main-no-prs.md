...
- No review safety net — bad commits land on `main`. Mitigated by small commits, fast revert (`git revert <sha>` + redeploy from binary backup), and the `study-app.bak` rollback.
